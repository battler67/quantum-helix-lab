...
$env:PYTHONPATH='C:\Users\seera\Desktop\Exploration\Rit_Grover_project\qgsa_grover\src'; python -m pytest qgsa_grover\tests -q
cd Rit_Grover_project\quantum-helix-lab
npm run build
npm run lint
```
...
- Hybrid mode is limited to 32-base windows. Its fixed-point guarantee assumes
  at least one but not every position is marked; the all-match and all-mismatch
  extremes both remain uniform over positions and cannot be localized from the
  position histogram alone.
