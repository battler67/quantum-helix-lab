...
def build_hybrid_fixed_point_circuits(
    ...
    *,
    ...
    if len(reference) > MAX_HYBRID_LENGTH:
        raise ValueError(f"Hybrid mode is limited to {MAX_HYBRID_LENGTH} bases per circuit")
...
class HybridQuantumSearchEngine:
    ...
    def validate_request(self, query_sequence: str, target_sequence: str) -> None:
        ...
        if len(query_sequence) > MAX_HYBRID_LENGTH:
            raise ValueError(f"Hybrid mode supports at most {MAX_HYBRID_LENGTH} bases per window")
